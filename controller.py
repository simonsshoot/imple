...
class TabletopController:
	...
	def _canonical_object_name(obj_name: str) -> str:
		...
		synonyms = {
			...
			"red tray": "red_bin",
			"red container": "red_bin",
			"red box": "red_bin",
			"blue tray": "blue_bin",
			"blue container": "blue_bin",
			"blue box": "blue_bin",
			"green tray": "green_bin",
			"green container": "green_bin",
			"green box": "green_bin",
		}
	...
	def _resolve_entity(self, obj_name: str, obj_num: Optional[int] = None) -> Optional[EntityRef]:
		...
		# Handle coloured bin/tray aliases (PartsSorting env)
		color_bin_aliases = {
			"redbin": "red_bin", "redtray": "red_bin", "redcontainer": "red_bin", "redbox": "red_bin",
			"bluebin": "blue_bin", "bluetray": "blue_bin", "bluecontainer": "blue_bin", "bluebox": "blue_bin",
			"greenbin": "green_bin", "greentray": "green_bin", "greencontainer": "green_bin", "greenbox": "green_bin",
		}
		if q_norm in color_bin_aliases:
			target = self._normalize_name(color_bin_aliases[q_norm])
			for name, ent in self._scene_entities.items():
	...
	def llm_skill_interact(self, instruction: str) -> Dict[str, Any]:
		...
		if instruction.startswith("find "):
			...
			# "push cube left", "push the ball forward"
			push_str = re.sub(r"\b(the|a|an)\b", "", instruction[len("push "):]).strip()
			push_str = re.sub(r"\s+", " ", push_str)
			parts = push_str.rsplit(" ", 1)
			if len(parts) == 2 and parts[1] in self._DIRECTION_DELTAS:
				obj_name, direction = parts[0], parts[1]
			elif len(parts) == 2 and parts[0] in self._DIRECTION_DELTAS:
				direction, obj_name = parts[0], parts[1]
			else:
				obj_name = push_str
				direction = "forward"
			obj_name, obj_num = self.extract_number_from_string(obj_name)
			ret = self.push(obj_name, direction, obj_num)
		elif instruction.startswith("pull "):
			pull_str = re.sub(r"\b(the|a|an)\b", "", instruction[len("pull "):]).strip()
			pull_str = re.sub(r"\s+", " ", pull_str)
			parts = pull_str.rsplit(" ", 1)
			if len(parts) == 2 and parts[1] in self._DIRECTION_DELTAS:
				obj_name, direction = parts[0], parts[1]
			elif len(parts) == 2 and parts[0] in self._DIRECTION_DELTAS:
				direction, obj_name = parts[0], parts[1]
				...
				direction = "back"
			obj_name, obj_num = self.extract_number_from_string(obj_name)
			ret = self.pull(obj_name, direction, obj_num)
		elif instruction.startswith("nudge "):
			nudge_str = re.sub(r"\b(the|a|an)\b", "", instruction[len("nudge "):]).strip()
			nudge_str = re.sub(r"\s+", " ", nudge_str)
			parts = nudge_str.rsplit(" ", 1)
			if len(parts) == 2 and parts[1] in self._DIRECTION_DELTAS:
				obj_name, direction = parts[0], parts[1]
			elif len(parts) == 2 and parts[0] in self._DIRECTION_DELTAS:
				direction, obj_name = parts[0], parts[1]
				...
				direction = "forward"
			obj_name, obj_num = self.extract_number_from_string(obj_name)
			ret = self.nudge(obj_name, direction, obj_num)
		else:
	...
	# --- Direction helper for push/pull/nudge ---
	_DIRECTION_DELTAS = {
		"left": np.array([0.0, 0.10, 0.0], dtype=np.float32),
		"right": np.array([0.0, -0.10, 0.0], dtype=np.float32),
		"forward": np.array([0.10, 0.0, 0.0], dtype=np.float32),
		"back": np.array([-0.10, 0.0, 0.0], dtype=np.float32),
	}

	def _compliant_move_to(
		self,
		...
		"""Like ``_move_to`` but with low gain and more steps for soft contact."""
		target_xyz = np.asarray(target_xyz, dtype=np.float32)
		for _ in range(steps):
			ee = self._get_tcp_pos()
			delta = np.clip((target_xyz - ee) * gain, -0.5, 0.5)
			action = self._build_action(delta_xyz=delta, grip=grip)
			self._step(action)
			if np.linalg.norm(target_xyz - ee) < tol:
				break

	def push(self, obj_name: str, direction: str, obj_num: Optional[int] = None) -> str:
		"""Approach object from the opposite side, then push it with compliant contact."""
		ent = self._resolve_entity(obj_name, obj_num)
		if ent is None:
			return f"Cannot find {obj_name} to push"

		direction = direction.strip().casefold()
		d = self._DIRECTION_DELTAS.get(direction)
		if d is None:
			return f"Invalid push direction: {direction}. Use left/right/forward/back."

		pos = self._get_entity_pos(ent)

		# 1) Approach from opposite side, slightly above table
		approach = pos - d * 1.2  # come from behind
		approach[2] = pos[2] + 0.015  # slight hover above object center height
		self._move_to(approach, grip=1.0, steps=36)

		# 2) Push through with low gain (Path B: compliant motion)
		push_target = pos + d * 1.5
		push_target[2] = pos[2] + 0.015
		self._compliant_move_to(push_target, grip=1.0, steps=80, gain=2.0)

		# 3) Retract upward
		ee = self._get_tcp_pos()
		self._move_to(ee + np.array([0.0, 0.0, 0.08], dtype=np.float32), grip=1.0, steps=16)
		return ""

	def pull(self, obj_name: str, direction: str, obj_num: Optional[int] = None) -> str:
		"""Grasp object and drag it in the given direction with compliant motion, then release."""
		ent = self._resolve_entity(obj_name, obj_num)
		if ent is None:
			return f"Cannot find {obj_name} to pull"

		direction = direction.strip().casefold()
		d = self._DIRECTION_DELTAS.get(direction)
		if d is None:
			return f"Invalid pull direction: {direction}. Use left/right/forward/back."

		# 1) Pick the object first (reuse existing pick logic)
		if self.held_object_name is None:
			ret = self.pick(obj_name, obj_num)
			if ret:
				return ret

		# 2) Drag with low gain
		ee = self._get_tcp_pos()
		pull_target = ee + d * 1.2
		pull_target[2] = max(0.06, float(pull_target[2]))
		self._compliant_move_to(pull_target, grip=-1.0, steps=60, gain=2.0)
	...
	def nudge(self, obj_name: str, direction: str, obj_num: Optional[int] = None) -> str:
		"""Short, gentle tap on object in the given direction."""
		ent = self._resolve_entity(obj_name, obj_num)
		if ent is None:
			return f"Cannot find {obj_name} to nudge"

		direction = direction.strip().casefold()
		d = self._DIRECTION_DELTAS.get(direction)
		if d is None:
			return f"Invalid nudge direction: {direction}. Use left/right/forward/back."

		pos = self._get_entity_pos(ent)

		# 1) Approach from opposite side
		approach = pos - d * 0.6
		approach[2] = pos[2] + 0.015
		self._move_to(approach, grip=1.0, steps=30)

		# 2) Short compliant tap (smaller distance than push)
		tap_target = pos + d * 0.5
		tap_target[2] = pos[2] + 0.015
		self._compliant_move_to(tap_target, grip=1.0, steps=30, gain=2.5)
