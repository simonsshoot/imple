...
def configure_args(args: argparse.Namespace) -> argparse.Namespace:
  ...
  if not str(getattr(out, "control_mode", "")).strip():
    # Humanoid tasks are typically joint-space controlled.
    out.control_mode = "pd_joint_pos"
  return out


def control_mode_candidates(args: argparse.Namespace) -> List[str]:
  preferred = str(getattr(args, "control_mode", "") or "pd_joint_pos").strip()
  return [preferred, "pd_joint_delta_pos", "pd_ee_delta_pose"]
